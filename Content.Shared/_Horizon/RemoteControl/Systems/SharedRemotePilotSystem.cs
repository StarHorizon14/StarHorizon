...
public abstract class SharedRemotePilotSystem : EntitySystem
{
    ...
    public bool TryCreateRemotePilot(EntityUid mech, EntityUid controller, [NotNullWhen(true)] out EntityUid? pilotUid)
    {
        ...
        if (TryComp<NpcFactionMemberComponent>(controller, out var faction))
        {
            EnsureComp<NpcFactionMemberComponent>(pilotUid.Value, out var pilotfaction);
            _npcFaction.AddFactions((pilotUid.Value, pilotfaction), faction.Factions);
        }
