...
public sealed partial class NFCargoConsoleMenu : FancyWindow
{
    ...
    public IEnumerable<CargoProductPrototype> ProductPrototypes
    {
        get
        {
            ...
            var nfConsole = _entityManager.GetComponentOrNull<NFCargoOrderConsoleComponent>(_owner);
            ...
            foreach (var cargoPrototype in _protoManager.EnumeratePrototypes<CargoProductPrototype>())
            {
                ...
                if (nfConsole is { RandomizeProducts: true } && !nfConsole.AllowedProductIds.Contains(cargoPrototype.ID))
                    continue;
    ...
    public void PopulateProducts()
    {
        ...
        foreach (var prototype in products)
        {
                ...
                search.Length == 0 && _category != null && Loc.GetString(prototype.Category).Equals(_category))
            {
                ...
                var discountPercent = IoCManager.Resolve<IUserInterfaceManager>()
                    .GetUIController<SponsorShopUIController>().DiscountPercent;
                if (discountPercent > 0)
                    cost = (int)(cost * (1 - discountPercent / 100f));

                var costText = BankSystemExtensions.ToSpesoString(cost);
                if (discountPercent > 0)
                    costText += $" (-{discountPercent}%)";
                // Horizon end
                ...
                var button = new CargoProductRow
                {
                    ...
                    PointCost = { Text = costText }, // Frontier:
